...
router = APIRouter(
    prefix="/module/v1/order_commodity_detail",
    tags=["order_commodities_detail"],
)

@router.get("/all", response_model=List[schemas.Response])
async def get_all_order_commodities_detail(db: Session = Depends(get_db)):
    data_order_commodities_detail = db.query(models.OrderCommodityDetail).all()
    if not data_order_commodities_detail:
        raise HTTPException(status_code=404, detail="Không tìm thấy chi tiết phiếu nhập nào!")
...
async def get_order_commodity_detail(ma_pn: str, db: Session = Depends(get_db)):
    data_order = db.query(models.OrderCommodityDetail).filter(models.OrderCommodityDetail.ma_pn == ma_pn).all()
    if not data_order:
        raise HTTPException(status_code=404, detail="Chi tiết phiếu nhập với mã {ma_pn} không tìm thấy!")
    return data_order
...
async def create_order_commodity_detail(data: schemas.Request, db: Session = Depends(get_db)):
    ...
    if not data_order_commodity:
        raise HTTPException(status_code=404, detail="Chi tiết phiếu nhập với mã {data.ma_pn} không tìm thấy! ")
